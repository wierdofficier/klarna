...
/* create solution from the data , den är korrekt.*/
while(count < 36*5)
{
	double A = Z[count] ;
...
for(int cc = 1; cc < 4; cc++)
{
	  ...
	  newZ = ((( (XX) + (-Y - (XX))/(1.0 + pow((0.1/(D)),2.2309)   )  )));
 if(cc == 2)
 	  printf("%.10f %.10f %.10f  %.10f  %.10f      \n", f,newZ,XX,Y, D   );
volt+=0.1;
}
...
while(count <  36*5)
{
	...
	XX=314.106 - 3.92858e-12*  ( (zz));//191.712 - 5.06738e-12 *sqrt(866259593727648335845654528.0 + 1544930458980476393965211067875328.0* r);
//	XX=1091.38 - 1.0857e-10 *sqrt(121584890983464837366939648.0 - 3206493838972879002136444141568.0* rr);
 //-191.712 - 5.06738e-12 *sqrt(866259593727648335845654528.0 + 1544930458980476393965211067875328.0* f);
	...
	double D =  -5.367391155e-5 *pow(XX,2.0) + 1.045223119e-2 *XX + 1.634736817e-1;

	double L =  4.693490519e-5* pow(XX,2.0) - 1.510618255e-2* XX+ 7.420876305e-1; 

	double textZ;
	   textZ   = ((( (XX) + (-L - (XX))/(1.0 + pow((0.1/(D)),2.2309)   )  )));;
	 //printf("%.10f %.10f %.10f\n", f,newZ,Z[count]   );
